...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: oliver-nyx/coherencelab@v1.9.3
        with:
...
| `version` | no | `v1.9.3` | Release tag/branch to clone (ignored if `local-source` set) |
...
This repository is the action (`action.yml` at root). Tag releases (`v1.9.3`) so consumers can pin `@v1.9.3` or `@v1`.
