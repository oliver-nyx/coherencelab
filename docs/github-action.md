...
| `version` | no | `v1.9.0` | Release tag/branch to clone (ignored if `local-source` set) |
...
This repository is the action (`action.yml` at root). Tag releases (`v1.9.0`) so consumers can pin `@v1.9.0` or `@v1`.
