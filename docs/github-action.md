...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: oliver-nyx/coherencelab@v1.9.4
        with:
...
| `version` | no | `v1.9.4` | Release tag/branch to clone (ignored if `local-source` set) |
...
This repository is the action (`action.yml` at root). Tag releases (`v1.9.4`) so consumers can pin `@v1.9.4` or `@v1`.
